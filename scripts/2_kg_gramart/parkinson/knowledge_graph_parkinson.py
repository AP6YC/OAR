...
It creates a knowledge graph based on variants of Charcot-Marie-Tooth disease
Cases are from the Parkinsons phenotypic series in the OMIM (On-Line Mendelian Inheritance of Man)
The graph is a bipartite graph with major nodes as disease variants (n=81) and genes (n=64)
The disease variants are in the disease_list (n=31)
The genes are in the gene_list 
...
The inheritance list (n=4) lists the four modes of genetic inheritance [AR,AD, XLD, and XLR, and Mu (multifactorial) 
Multiple inheritances are separated by pipe character
Multiple genes are separated by pipe character
All spaces converted to underscore
All hypens and commas converted to underscore character
"""
...
os.chdir('/Users/danielhier/desktop/parkinson')
# G is the Graph object for Parkinson variants
G = nx.DiGraph()
phenotype_list=[]
gene_list=[]
gene_protein_map=[]
...
gene=''
proteins=set()
df= pd.read_csv("Parkinson_OMIM.csv", header=0)
...
################################################################################################################
parkinson_variants = df.values.tolist()  # Converts df to a list of 31 variants
#Create supernodes
...
#Add nodes and edges from file Parkinson_genes.csv
#Add gene data from "Parkinson_genes.csv"

gene_locations=[]
for d in parkinson_variants:
    gene_location = d[0]
    disease=d[1]
    disease_MIM = d[2]
    gene=d[3]
    ...
    G.add_node(gene, category='gene', MIM = gene_MIM, class_type='individual') # add each gene as a node
   ...
   #      print(disease, disease_MIM)
         if disease_MIM in disease_MIMs:  #Disease MIMs is a list of 81 variants of CMT
             for d in disease_list:
                 d_MIM =G.nodes[d].get('MIM',0)
                 if disease_MIM == d_MIM:
                     disease = d
             for h in hpo_tags:
               hpo=h[0]
               phenotype = h[1]
...
# Gene |Protein Name   |	Uniprot_num     |  Chromosome |Protein Class |	Biologic process |Molecular function |Disease involvement | Protein_location |  MW  | length|domain| motif        #      
##############################################################################################################################################################################################

df_parkinson_proteins =pd.read_csv('protein_list_parkinson.csv')
parkinson_proteins = df_parkinson_proteins.values.tolist()
for p in parkinson_proteins:
    gene=p[0]
    protein =p[1]
    uniprot = p[2]
    chromosome=p[3]
    ...
    location=p[8]
    length=p[10]
    G.add_node(chromosome, class_type='individual', category ='chromosome')
    ...
    if 'none' not in protein_domain:
            for p_d in protein_domain:
                ...
                G.add_edge(protein, p_d, relation ='has_a_protein_domain')
    ...
    if 'none' not in protein_motif:
            for pm in protein_motif:
                ...
                G.add_edge(protein, pm, relation='has_a_protein_motif')
        
    protein_location=location.split('|')
    if 'none' not in protein_location:
            for pl in protein_location:
...
nx.write_graphml(G, 'parkinson.graphml')
...
file_out ='parkinson.owl'
g.serialize(file_out, format="xml")
# Print confirmation message
print("Parkinson Ontology written to", file_out)
...
for node, attributes in G.nodes.data():
  #  print(f"Node {node}: {attributes}")
    if G.nodes[node].get('class_type',0)==0:
        print(node, G.nodes[node].get('class_type',0))   
...
with open('node_attributes_parkinson.txt', 'w') as file:
...
# Open a file in write mode
with open('edge_attributes_parkinson.txt', 'w') as file:
